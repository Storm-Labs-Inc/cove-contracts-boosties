...
import { IFactory } from "src/interfaces/deps/yearn/vault-periphery/IFactory.sol";
...
contract Registry is Governance {
    ...
    event NewEndorsedStrategy(address indexed strategy, address indexed asset, uint256 releaseVersion);

    event RemovedVault(address indexed vault, address indexed asset, uint256 releaseVersion);

    event RemovedStrategy(address indexed strategy, address indexed asset, uint256 releaseVersion);
     ...
     */
    function numEndorsedStrategies(address _asset) public view returns (uint256) {
     ...
     */
    function getAllEndorsedVaults() external view returns (address[][] memory allEndorsedVaults) {
     ...
     */
    function getAllEndorsedStrategies() external view returns (address[][] memory allEndorsedStrategies) {
        address[] memory allAssets = assets;
        uint256 length = assets.length;
        ...
        returns (address _vault)
    {
        ...
        uint256 _releaseTarget = ReleaseRegistry(releaseRegistry).numReleases() - 1 - _releaseDelta;

        // Get the factory address for that specific Api version.
        address factory = ReleaseRegistry(releaseRegistry).factories(_releaseTarget);

        // Make sure we got an actual factory
        require(factory != address(0), "Registry: unknown release");

        // Deploy New vault.
        _vault = IFactory(factory).deploy_new_vault(ERC20(_asset), _name, _symbol, _roleManager, _profitMaxUnlockTime);
     ...
     */
    function endorseVault(address _vault, uint256 _releaseDelta, uint256 _deploymentTimestamp) public onlyGovernance {
        // Will underflow if no releases created yet, or targeting prior to release history
        uint256 releaseTarget = ReleaseRegistry(releaseRegistry).numReleases() - 1 - _releaseDelta; // dev: no releases

        // Get the API version for the target specified
        string memory apiVersion = IFactory(ReleaseRegistry(releaseRegistry).factories(releaseTarget)).api_version();

        require(keccak256(bytes(IVault(_vault).api_version())) == keccak256(bytes((apiVersion))), "Wrong API Version");

        // Add to the end of the list of vaults for asset
        _registerVault(_vault, IVault(_vault).asset(), releaseTarget, _deploymentTimestamp);
    }
        ...
        internal
    {
        ...
        info[_vault] =
            Info({ asset: _asset, releaseVersion: _releaseTarget, deploymentTimestamp: _deploymentTimestamp, tag: "" });
    ...
    {
        // Will underflow if no releases created yet, or targeting prior to release history
        uint256 _releaseTarget = ReleaseRegistry(releaseRegistry).numReleases() - 1 - _releaseDelta; // dev: no releases

        // Get the API version for this release
        string memory apiVersion = IFactory(ReleaseRegistry(releaseRegistry).factories(_releaseTarget)).api_version();

        // Make sure the API versions match
        require(
            keccak256(bytes((IStrategy(_strategy).apiVersion()))) == keccak256(bytes((apiVersion))), "Wrong API Version"
        );
     ...
     */
    function tagVault(address _vault, string memory _tag) external onlyGovernance {
     ...
     */
    function removeVault(address _vault, uint256 _index) external onlyGovernance {
        require(info[_vault].asset != address(0), "!endorsed");

        // Get the asset the vault is using.
        address asset = IVault(_vault).asset();
        // Get the relase version for this specific vault.
        uint256 releaseTarget = ReleaseRegistry(releaseRegistry).releaseTargets(IVault(_vault).api_version());

        require(_endorsedVaults[asset][_index] == _vault, "wrong index");

        // Set the last index to the spot we are removing.
        _endorsedVaults[asset][_index] = _endorsedVaults[asset][_endorsedVaults[asset].length - 1];
     ...
     */
    function removeStrategy(address _strategy, uint256 _index) external onlyGovernance {
        require(info[_strategy].asset != address(0), "!endorsed");

        // Get the asset the vault is using.
        address asset = IStrategy(_strategy).asset();
        // Get the relase version for this specific vault.
        uint256 releaseTarget = ReleaseRegistry(releaseRegistry).releaseTargets(IStrategy(_strategy).apiVersion());

        require(_endorsedStrategies[asset][_index] == _strategy, "wrong index");

        // Set the last index to the spot we are removing.
        _endorsedStrategies[asset][_index] = _endorsedStrategies[asset][_endorsedStrategies[asset].length - 1];
     ...
     */
    function removeAsset(address _asset, uint256 _index) external onlyGovernance {
        require(assetIsUsed[_asset], "!in use");
        require(_endorsedVaults[_asset].length == 0 && _endorsedStrategies[_asset].length == 0, "still in use");
