...
// https://github.com/pcaversaccio/snekmate/blob/690dc0710aa0c3fd80f707ebf6dd50674f3f7ff1/lib/utils/VyperDeployer.sol
...
contract VyperDeployer is Create {
    address private constant _HEVM_ADDRESS = address(uint160(uint256(keccak256("hevm cheat code"))));
     ...
     */
    _CheatCodes private _cheatCodes = _CheatCodes(_HEVM_ADDRESS);
    ...
    function deployContract(string memory path, string memory fileName) public returns (address) {
         ...
         */
        if (deployedAddress == address(0)) revert DeploymentFailed(_self);
