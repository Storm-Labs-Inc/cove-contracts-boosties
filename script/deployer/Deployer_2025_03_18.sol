...
contract Deployer20250115 is BaseDeployScript, CurveSwapParamsConstants {
    ...
    function deploy() public override {
        ...
        _deployCoveStrategy(
            deployer.getAddress("YearnStakingDelegate"), MAINNET_YVUSDS_1_GAUGE, getMainnetYvusdsGaugeCurveSwapParams()
        );

        // TODO: execute these as multisig transactions
        // Deploy the cove gauges from admin address
        _deployCoveGauges(MAINNET_YVUSDS_1_GAUGE);
        // Set roles and deposit limit from timelock address
        // Limit deposit to max 100,000 USD
        _setRolesAndDepositLimit(MAINNET_YVUSDS_1_GAUGE, 100_000e18);
        // Approve tokens in the router
        address[] memory newYearnGauges = new address[](1);
        newYearnGauges[0] = MAINNET_YVUSDS_1_GAUGE;
        ...
        deployIfMissing(string.concat("YearnGaugeStrategy-", IERC4626(yearngauge).name()))
    {
        YearnGaugeStrategy strategy = deployer.deploy_YearnGaugeStrategy(
            string.concat("YearnGaugeStrategy-", IERC4626(yearngauge).name()),
            yearngauge,
            ysd,
            MAINNET_CURVE_ROUTER_NG_V1_2
        );
        vm.startBroadcast();
